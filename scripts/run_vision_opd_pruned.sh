...
# =============================================================================
# Vision-OPD Training Script with Pre-LLM Token Pruning
...
TEACHER_UPDATE_RATE=0.0 # 0.0 disables EMA updates, keeping the teacher model permanently frozen
...
# PPO max token length per GPU: set to 2x the maximum sequence length as requested (fits roughly 2 sequences per GPU)
PPO_MAX_TOKEN_LEN_PER_GPU=$((TRAIN_MAX_MODEL_LEN * 2))

# Alternative fixed batch size option if dynamic batching is disabled:
# ACTOR_MICRO_BATCH_SIZE=2

# --- Pre-LLM Vision Token Pruning Parameters ---
KEEP_RATIO=0.5
PRUNING_METHOD="greedy_prune"  # options: greedy_prune, random, uniform, text_saliency

# --- Offloading & Memory ---
# Note: For the 3B model on 8 GPUs, FSDP sharding is sufficient to fit all weights in GPU memory.
# Setting offload to False avoids PCIe transfer overheads and speeds up training by 2x - 3x.
...
# --- Data Paths (Default Dataset: ChartQA) ---
DATA_DIR="${PROJECT_ROOT}/data"
TASK_TRAIN_FILE="${DATA_DIR}/chartqa_train.parquet"

MODEL_NAME=$(basename "$MODEL_PATH")
EXPERIMENT_NAME="Vision-OPD-${MODEL_NAME}-pruned"
...
echo "Max Pixels: $MAX_PIXELS (approx 3000 tokens max)"
echo "Max Prompt/Response Length: $MAX_PROMPT_LENGTH / $MAX_RESPONSE_LENGTH"
...
python3 -m verl.trainer.main_ppo --config-name "$CONFIG_NAME" \
    ...
    +actor_rollout_ref.model.vision_token_pruning.method=$PRUNING_METHOD \
