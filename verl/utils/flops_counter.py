...
def _estimate_qwen2_flops(config, tokens_sum, batch_seqlens, delta_time):
    # Qwen2.5-VL wraps the language-model configuration in ``text_config``
    # (Transformers >= 5), while plain Qwen2/Llama configs expose these fields
    # at the top level.  Keep the FLOPs accounting compatible with both shapes;
    # this counter is diagnostic and must not make an otherwise valid training
    # step fail after the actor update has completed.
