...
@contextmanager
def _init_empty_weights_compat():
    """Accelerate init_empty_weights variant compatible with torch Parameter attrs.

    Newer transformers may attach attributes such as ``_is_hf_initialized`` to
    parameters. accelerate forwards ``Parameter.__dict__`` into the Parameter
    constructor, but torch Parameter only accepts ``data`` and ``requires_grad``.
    Keep those attrs by setting them after constructing the meta parameter.
    """
    ...
    def register_empty_parameter(module, name, param):
        ...
        param_attrs = dict(getattr(old_param, "__dict__", {}))
        new_param = type(old_param)(old_param.to(device), requires_grad=param.requires_grad)
