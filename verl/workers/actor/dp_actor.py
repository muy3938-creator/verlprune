    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if "vision_token_keep_mask" in multi_modal_inputs:
                ...
                # apply_pre_llm_pruning_to_attention_mask expects a list of per-sample dicts
                mm_inputs_list = micro_batch.get("multi_modal_inputs", [])
                if not isinstance(mm_inputs_list, list):
                    mm_inputs_list = [mm_inputs_list]
                attention_mask = apply_pre_llm_pruning_to_attention_mask(
                    input_ids, attention_mask, mm_inputs_list, image_token_id
                )
